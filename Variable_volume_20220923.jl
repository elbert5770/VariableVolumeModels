    ...
    for j = 1:2*number_of_days
        ...
        for k = 0:1
            ...
            for t = 0.0:delt:tmax
                ...
                # ##### Solution calculated from dy/dt = P + RV^(-1)y with integrating factor
                # ##### Uses ExponentialUtilities.jl
                # ##### Seems less stable than Base.exp() for drain
                # if solution_method == 2
                #     integral, err = quadgk(x -> calcexpnegRintinvVP(x,p,R,P), 0.0, t)
                #     x = inv(V)*expv(1.0,calcRintinvV(t,p,R),integral + V0*x0 )
                #     y = expv(1.0,calcRintinvV(t,p,R),integral + y0 )
                #     # @show t, calcRintinvV(t,p,R) expRintinvV, calcexpnegRintinvV(t,p,R),integral,inv(V),integral*P 
                # end
                # #####
...
function calcRintinvV(t,p,R)
 ...
 function calcexpnegRintinvV(t,p,R)
     ...
     soln = eigen(-calcRintinvV(t,p,R))
     @show t,soln.values
     return exp(-calcRintinvV(t,p,R))
 end
 
 function calcexpnegRintinvVP(t,p,R,P)
    # Calculate exp(-R*Integrate[inv(V)])
    return expv(-1.0,calcRintinvV(t,p,R),P)
end
